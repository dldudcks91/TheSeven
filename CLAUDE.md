...
기획서를 받으면 아래 항목을 채워 개발 기획서를 작성한다:
...
#### 변환 관점
기획서를 읽을 때 아래 질문을 통해 개발 범위를 결정한다:
...
**3. pytest 통합 테스트**
- `tests/` 디렉토리에 해당 도메인 테스트 파일 작성/수정
- `python -m pytest tests/ -v` 실행하여 정상/실패 흐름 검증
- 테스트 인프라: theseven_test DB + fakeredis + httpx AsyncClient
- 정상 흐름 + 실패 흐름 (success: false) 응답 확인
- 기존 테스트 회귀 확인 (전체 테스트 스위트 실행)
...
아래 항목을 `.claude/docs/버전관리.md`에 한 줄 요약으로 추가한다:
- 작업 내용
- 변경된 파일 목록
...
#### 3. 문서 업데이트 확인
Step 3 체크리스트의 `.claude/docs/API.md` 업데이트가 완료되었는지 최종 확인한다.
...
| `.claude/docs/GDD.md` | 게임 기획서 (전 시스템 구현 현황 포함) |
| `.claude/docs/API.md` | 전체 API 명세 |
| `fastapi/models.py` | DB 스키마 |
| `fastapi/services/system/` | APIManager 라우팅 테이블 |
| `.claude/docs/BACKEND.md` | Manager 목록, Redis 키, Worker 구조 |
| `.claude/docs/COMBAT.md` | 전투/NPC 시스템 상세 |
| `.claude/docs/CLIENT.md` | 클라이언트 구조 |
| `.claude/docs/버전관리.md` | 버전별 구현 내역 |
...
| 수정 대상 | 먼저 읽을 파일 |
|-----------|---------------|
| 새 API 추가 | `.claude/docs/API.md`, `services/system/api_manager.py` |
...
| 테스트 추가/수정 | `tests/conftest.py`, 해당 `tests/test_{domain}.py` |
...
```
fastapi/tests/
├── conftest.py           # 테스트 설정 (DB/Redis fixture, AsyncClient)
├── test_building.py      # 건물/자원 API 테스트 (샘플)
└── ...                   # 도메인별 테스트 파일 추가
```
