...
```
┌──────────────────────────────────────────────────────────────┐
│ 헤더: 플레이어 ID | 전체 데이터 로드 | 신규 유저 생성 | 국가정보 | 현재 시각 │
├──────────────────────────────────────────────────────────────┤
│ 탭: [API 테스트] [게임] [⚔️ 영웅] [연맹] [전투] [전장]          │
├──────────────────────────────────────────────────────────────┤
...
**헤더 기능:**
- 플레이어 ID 입력 + "전체 데이터 로드" 버튼: 서버 캐시 프리로드(1010) + WS 연결 + iframe 전파
- "신규 유저 생성" 버튼: API 1003 호출 → 생성된 user_no 자동 입력
- 국가 정보 표시: API 1009 호출 → 닉네임 | Lv | 전투력 | 좌표 표시
...
| hero | ⚔️ 영웅 | hero.html iframe 1개 |
| ally | 연맹 | alliance.html iframe 1개 |
| battle | 전투 | battle.html iframe 1개 (맵/행군/NPC) |
| battlefield | 전장 | battlefield.html iframe 1개 |
...
| heroFrame | hero.html | 영웅 목록/지급 |
| allyFrame | alliance.html | 연맹 관리 |
| battleFrame | battle.html | 맵/행군/NPC 사냥/전투 보고서 |
| battlefieldFrame | battlefield.html | 전장 참여/관전 |
...
2. 유저가 "전체 데이터 로드" 버튼 클릭
   ...
   ├─ POST /api { api_code: 1009 }  → 국가 정보 조회 → 헤더에 표시
   ├─ WebSocket 연결 ws://{host}/ws/{userId}
   └─ 모든 iframe(12개)에 postMessage { type: 'set_player_id', userId }
       └─ 각 iframe이 자체 API 호출로 데이터 로드

3. "신규 유저 생성" 버튼 클릭
   └─ POST /api { api_code: 1003 }  → 생성된 user_no를 globalUserId에 자동 입력
```
...
| 파일명 | API 코드 | 주요 기능 |
|--------|---------|---------|
| main.html | 1002, 1003, 1009, 1010 | 메인 컨테이너, WebSocket, 설정 로드, 신규 유저 생성, 국가 정보 |
| resource.html | 1011 | 자원 현황 표시 (상단 바) |
| buff.html | 1012, 1013, 1014 | 버프 현황 (상세/총합/타입별 필터) |
| building.html | 2001~2007 | 건물 목록, 건설/업그레이드/취소/가속 |
| research.html | 3001~3004 | 연구 트리, 연구 시작/취소 |
| unit.html | 4001~4006 | 유닛 목록, 훈련/업그레이드/완료/취소/가속 |
| item.html | 6001~6003 | 아이템 인벤토리, 획득(테스트), 사용 |
| mission.html | 5001~5002 | 미션 목록, 보상 수령 |
| shop.html | 6011~6013 | 상점 진열, 새로고침, 구매 |
| hero.html | 8001, 8002 | 영웅 목록, 영웅 지급(테스트) |
| alliance.html | 7001~7017 | 연맹 전체 관리 (상세는 아래) |
| battle.html | 4001, 8001, 9001~9003, 9011~9013, 9022 | 맵, 행군, NPC 사냥, 전투 보고서 |
| battlefield.html | 9050~9055 | 전장 목록, 참여/후퇴, 관전 |
...
- [ ] `main.html`의 `frameIds` 배열(현재 12개)에 ID 추가 (`set_player_id` 자동 전파용)
