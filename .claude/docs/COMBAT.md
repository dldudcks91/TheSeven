...
✅ Phase 6a: 성 일반공격 (multi-attacker + 무혈입성)
📋 Phase 6b: 성 집결공격 (구현 예정)
...
- 수비자 자원: Redis 즉시 차감
- 수송 용량: 무제한 (추후 확장 예정)
- 다중 공격자 약탈 분배: 기여도(damage_dealt) 비례. `int()` 절삭으로 분배 합계 < 총 약탈량 가능 (1~2 오차)
...
| 항목 | 내용 | 상태 |
|------|------|------|
| 성 공격 max_rounds | ~~현재 50~~ → 라운드 제한 없음으로 확정 | ✅ 확정 |
| 약탈 수송 용량 | 무제한으로 확정 (추후 확장 예정) | ✅ 확정 |
| 다중 공격자 약탈 분배 | 기여도(damage_dealt HP 피해) 비례 분배로 확정 | ✅ 확정 |
| 집결 동시 참여 제한 | 플레이어당 동시 집결 참여 가능 수 | 미결정 |
| 집결 취소 규칙 | Leader 취소 시 이동 중인 rally_gather 병력 처리 | 미결정 |
| 병종 상성 수치 | 계수 1.5× 확정 여부 | 미결정 |
| NPC 집결 EXP 분배 | 참여자 간 EXP 배분 방식 | 미결정 |
| 거점 점령 로직 | 점령 시간, 연맹 귀속, 거점 버프 효과 | 미결정 |
