...
class BaseRedisCacheManager:
    """Redis 캐시 관리 기본 클래스 - Hash 및 일반 캐싱 지원"""
    ...
    def set_data(self, key: str, data: Any, expire_time: Optional[int] = None) -> bool:
        """일반 데이터 캐싱"""
        try:
    ...
    def extend_ttl(self, key: str, expire_time: Optional[int] = None) -> bool:
        """TTL 연장"""
        try:
            expire_time = expire_time or self.default_expire_time
    ...
    def set_hash_data(self, hash_key: str, data: Dict[str, Any], 
    ...
    def get_hash_data(self, hash_key: str) -> Optional[Dict[str, Any]]:
        """Hash 구조 데이터 전체 조회"""
        try:
    ...
    def set_hash_field(self, hash_key: str, field: str, value: Any, 
                       expire_time: Optional[int] = None) -> bool:
        """Hash의 특정 필드 설정"""
        try:
            expire_time = expire_time or self.default_expire_time
    ...
    def get_hash_field(self, hash_key: str, field: str) -> Optional[Any]:
        """Hash의 특정 필드 조회"""
        try:
            value = self.redis_client.hget(hash_key, str(field))
    ...
    def delete_hash_field(self, hash_key: str, field: str) -> bool:
        """Hash의 특정 필드 삭제"""
        try:
            result = self.redis_client.hdel(hash_key, str(field))
    ...
    def get_hash_fields(self, hash_key: str, fields: List[str]) -> Dict[str, Any]:
        """Hash의 여러 필드 한번에 조회"""
        try:
            values = self.redis_client.hmget(hash_key, fields)
    ...
    def get_hash_length(self, hash_key: str) -> int:
        """Hash의 필드 개수 조회"""
        try:
    ...
    def hash_exists_field(self, hash_key: str, field: str) -> bool:
        """Hash에 특정 필드 존재 여부 확인"""
        try:
            return bool(self.redis_client.hexists(hash_key, str(field)))
        except Exception as e:
    ...
    def set_multiple(self, data_dict: Dict[str, Any], expire_time: Optional[int] = None) -> bool:
        """여러 키-값 쌍을 한번에 설정"""
        try:
            expire_time = expire_time or self.default_expire_time
    ...
    def get_keys_by_pattern(self, pattern: str) -> List[str]:
        ...
        try:
            ...
            self.logger.debug(f"Found {len(result)} keys matching pattern: {pattern}")
            return result
    ...
    def get_cache_stats(self, key_pattern: Optional[str] = None) -> Dict[str, Any]:
        """캐시 통계 정보 조회"""
        try:
            ...
            for key in keys[:100]:
                ...
                key_info = {
                    "key": key,
                    "type": self.redis_client.type(key).decode('utf-8') if isinstance(self.redis_client.type(key), bytes) else str(self.redis_client.type(key)),
                    "ttl": self.redis_client.ttl(key),
                    "memory_usage": self._get_memory_usage(key)
                }
    ...
    def _get_memory_usage(self, key: str) -> Optional[int]:
