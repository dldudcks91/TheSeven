...
class RedisManager:
    ...
    def get_all_queue_status(self) -> Dict[str, Dict[str, int]]:
        ...
        if self._building_manager:
            result['building'] = self._building_manager.get_queue_status()
        if self._unit_manager:
            result['unit_training'] = self._unit_manager.get_queue_status()
        if self._research_manager:
            result['research'] = self._research_manager.get_queue_status()
        if self._buff_manager:
            result['buff'] = self._buff_manager.get_queue_status()
    ...
    def get_all_completed_tasks(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if self._building_manager:
            result['building'] = self._building_manager.get_completed_tasks()
        if self._unit_manager:
            result['unit_training'] = self._unit_manager.get_completed_tasks()
        if self._research_manager:
            result['research'] = self._research_manager.get_completed_tasks()
        if self._buff_manager:
            result['buff'] = self._buff_manager.get_completed_tasks()
