...
class ResearchRedisManager(BaseRedisTaskManager):
    ...
    async def add_research(self, user_no: int, research_id: int, completion_time: datetime) -> bool:
        """연구를 큐에 추가"""
        if not self.validate_task_data(research_id):
            return False
        return await self.add_to_queue(user_no, research_id, completion_time)
    
    async def remove_research(self, user_no: int, research_id: int) -> bool:
        """연구를 큐에서 제거"""
        return await self.remove_from_queue(user_no, research_id)
    
    async def get_research_completion_time(self, user_no: int, research_id: int) -> Optional[datetime]:
        """연구 완료 시간 조회"""
        return await self.get_completion_time(user_no, research_id)
    ...
    async def speedup_research(self, user_no: int, research_id: int) -> bool:
        """연구 즉시 완료"""
        return await self.update_completion_time(user_no, research_id, datetime.utcnow())