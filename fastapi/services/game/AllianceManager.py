...
class AllianceManager:
    ...
    DB 동기화:
        ...
        - 데이터 변경 시 mark_dirty(alliance_no)로 동기화 대기열에 추가
    ...
    async def _update_alliance_buff_for_all_members(self, alliance_no: int, level: int):
        try:
            ...
            for user_no_str in members.keys():
                ...
                await self._remove_alliance_buff(user_no, alliance_no)
                await self._add_alliance_buff(user_no, alliance_no, level)
        except Exception as e:
            self.logger.error(f"Error updating buffs for all members: {e}")
    
    async def _check_level_up(self, alliance_no: int, current_exp: int, current_level: int) -> int:
        ...
        if new_level > current_level:
            await self._update_alliance_buff_for_all_members(alliance_no, new_level)
        return new_level

    # ==================== 연구 관련 헬퍼 ====================
    
    async def _check_research_level_up(self, alliance_no: int, research_idx: int,
        ...
        while True:
            ...
            if remaining_exp >= required_exp:
                ...
                break
        if new_level > current_level:
            await self._apply_research_buff(alliance_no, research_idx, new_level)
        return {"new_level": new_level, "remaining_exp": remaining_exp}
    
    async def _apply_research_buff(self, alliance_no: int, research_idx: int, level: int):
        try:
    ...
    async def alliance_info(self) -> Dict:
        ...
        try:
            ...
            if self._data and self._data.get('alliance_no'):
                alliance_no = self._data.get('alliance_no')
            else:
            ...
            if nation is None:
                nation = await nation_manager.get_nation(user_no)
            if nation and nation.get('alliance_no') == alliance_no:
                my_position = nation.get('alliance_position')
            
            notice_data = await alliance_redis.get_notice(alliance_no)
    ...
    async def alliance_join(self) -> Dict:
        ...
        try:
            ...
            alliance_no = self._data.get('alliance_no')
            if not alliance_no:
                return {"success": False, "message": "Missing alliance_no"}
            ...
            try:
                ...
                if join_type == self.JOIN_TYPE_FREE:
                    ...
                    await alliance_redis.add_member(alliance_no, user_no, member_data)
                    await nation_manager.set_user_alliance_info(user_no, alliance_no, self.POSITION_MEMBER)
                    await self._add_alliance_buff(user_no, alliance_no, alliance_info.get('level', 1))
                    await alliance_redis.mark_dirty(alliance_no)
                    return {"success": True, "data": {"status": "joined", "alliance_no": alliance_no}}
                else:
                    existing_app = await alliance_redis.get_application(alliance_no, user_no)
                    if existing_app:
                        return {"success": False, "message": "이미 가입 신청 중입니다"}
                    app_data = {"applied_at": now}
                    await alliance_redis.add_application(alliance_no, user_no, app_data)
                    await alliance_redis.mark_dirty(alliance_no)
                    return {"success": True, "data": {"status": "applied", "alliance_no": alliance_no}}
            finally:
                await alliance_redis.release_lock(alliance_no)
        except Exception as e:
    ...
    async def alliance_leave(self) -> Dict:
        ...
        try:
            ...
            try:
                await alliance_redis.remove_member(alliance_no, user_no)
                await nation_manager.clear_user_alliance_info(user_no)
                await self._remove_alliance_buff(user_no, alliance_no)
                await alliance_redis.mark_dirty(alliance_no)
    ...
    async def alliance_members(self) -> Dict:
        ...
        try:
            ...
            return {"success": True, "data": {"alliance_no": alliance_no, "members": member_list}}
        except Exception as e:
    ...
    async def alliance_kick(self) -> Dict:
        ...
        try:
            ...
            alliance_redis = self._get_alliance_redis()
            target_member = await alliance_redis.get_member(alliance_no, target_user_no)
            ...
            try:
                await alliance_redis.remove_member(alliance_no, target_user_no)
                await nation_manager.clear_user_alliance_info(target_user_no)
                await self._remove_alliance_buff(target_user_no, alliance_no)
                await alliance_redis.mark_dirty(alliance_no)
    ...
    async def alliance_promote(self) -> Dict:
        ...
        try:
            ...
            if new_position == self.POSITION_LEADER:
                ...
                try:
                    my_member_data = await alliance_redis.get_member(alliance_no, user_no)
                    my_member_data['position'] = self.POSITION_MEMBER
                    await alliance_redis.update_member(alliance_no, user_no, my_member_data)
                    await nation_manager.set_user_alliance_info(user_no, alliance_no, self.POSITION_MEMBER)
                    
                    target_member['position'] = self.POSITION_LEADER
                    await alliance_redis.update_member(alliance_no, target_user_no, target_member)
                    await nation_manager.set_user_alliance_info(target_user_no, alliance_no, self.POSITION_LEADER)
            ...
            try:
                ...
                await alliance_redis.update_member(alliance_no, target_user_no, target_member)
                await nation_manager.set_user_alliance_info(target_user_no, alliance_no, new_position)
                await alliance_redis.mark_dirty(alliance_no)
    ...
    async def alliance_applications(self) -> Dict:
        ...
        try:
            ...
            alliance_redis = self._get_alliance_redis()
            applications = await alliance_redis.get_applications(alliance_no)
            
            app_list = []
            for user_no_str, app_data in applications.items():
                app_list.append({"user_no": int(user_no_str), "applied_at": app_data.get('applied_at')})
            app_list.sort(key=lambda x: x['applied_at'])
            return {"success": True, "data": {"alliance_no": alliance_no, "applications": app_list}}
        except Exception as e:
    ...
    async def alliance_approve(self) -> Dict:
        ...
        try:
            ...
            alliance_redis = self._get_alliance_redis()
            application = await alliance_redis.get_application(alliance_no, target_user_no)
            if not application:
    ...
    async def alliance_donate(self) -> Dict:
        ...
        try:
            ...
            try:
                ...
                new_level = await self._check_level_up(alliance_no, new_exp, current_level)
                ...
                await alliance_redis.update_member(alliance_no, user_no, member_data)
        ...
        try:
            ...
            try:
                ...
                for member_user_no_str in members.keys():
                    ...
                    await nation_manager.clear_user_alliance_info(member_user_no)
                    await self._remove_alliance_buff(member_user_no, alliance_no)
                
                await alliance_redis.delete_all_alliance_data(alliance_no, alliance_name)
                await alliance_redis.mark_dirty(alliance_no)
    ...
    async def alliance_notice(self) -> Dict:
        ...
        try:
            ...
            alliance_redis = self._get_alliance_redis()
            notice_data = await alliance_redis.get_notice(alliance_no)
            return {"success": True, "data": {"notice": notice_data}}
        except Exception as e:
    ...
    async def alliance_notice_write(self) -> Dict:
        ...
        try:
            ...
            alliance_redis = self._get_alliance_redis()
            await alliance_redis.set_notice(alliance_no, notice_data)
    ...
    async def alliance_research_list(self) -> Dict:
        ...
        try:
            ...
            for research_idx, level_configs in research_configs.items():
                ...
                max_level = max(level_configs.keys()) if level_configs else 0
                is_active = (active_research and active_research.get('research_idx') == research_idx)
            ...
            return {"success": True, "data": {"research_list": research_list, "active_research": active_research}}
        except Exception as e:
